...
/**
 * 代表一个本地化字符串的唯一代码标识。
 * <p>一般来说此接口应该配合枚举来使用，使枚举实现此接口，然后使用单个枚举项来获取本地化字符串。</p>
 * <p>此接口可以作为函数式接口来使用。</p>
 * @author MARA
...
public interface LocalizedTag {
    ...
    default String getLocalizedString(Locale locale, Object... replacements) {
        ...
        String originalString = ResourceBundle.getBundle(localeFamily, locale, CONTROL).getString(key);
    ...
    static LocalizedTag of(String localeFamily, String tag) {
        if (localeFamily == null || tag == null || "".equals(localeFamily) || "".equals(tag)) {
